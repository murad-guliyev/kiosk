...
export function MoneyDetailPage() {
  ...
  // React Router keeps MoneyDetailPage mounted across prev/next/back transitions
  // (same route, different param). Strip any lingering exit animation class that
  // would otherwise leave the page stuck at opacity:0.
  useEffect(() => {
    const el = containerRef.current;
    if (el) el.classList.remove('anim-page-exit');
  ...
  const category = family ? categories.find((c) => c.id === family.categoryId) : undefined;

  if (!family) {
    return (
      <Layout backdrop={<AnimatedBg />}>
  ...
  return (
    <Layout backdrop={<AnimatedBg />}>
      <div className="flex-1 flex flex-col overflow-hidden" ref={(el) => { containerRef.current = el; }}>
        {/* Header bar — fixed above the scrollable content */}
        <div
          className="px-4 sm:px-6 lg:px-10 py-4 lg:py-5 anim-header-enter flex-shrink-0"
          style={{
        ...
        >
          <div className="flex items-center relative">
            <button
              onClick={handleBack}
              className="flex items-center gap-2.5 px-5 py-2.5 rounded-lg font-semibold anim-btn-enter"
              style={{
                background: 'linear-gradient(135deg, #4BC8B6 0%, #0B9ED0 100%)',
                border: '1px solid rgba(255,255,255,0.25)',
                color: '#fff',
                fontSize: '0.9rem',
                cursor: 'pointer',
                boxShadow: '0 4px 14px rgba(75,200,182,0.3), 0 0 0 1px rgba(75,200,182,0.12)',
              }}
            >
        ...
        <div className="flex-1 overflow-y-auto" ref={(el) => { scrollRef.current = el; }}>
